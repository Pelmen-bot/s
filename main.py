...
def text ():
    texxt = [
        ...
        "Согласно исследованию, проведенному в 2018 году, более 50% людей в возрасте от 18 до 34 летсчитают себя зависимыми от своих смартфонов.",
    ...
    ran_text = random.choice(texxt)
    return f"<h1>{random.choice(texxt)}</h1>"
...
def secret ():
    ...
    ran_secret = random.choice(monetka)
    return f"<h1>{random.choice(monetka)}</h1>"
app.run(debug=True)
